...
def compute_loss(model: nn.Module, x: torch.Tensor):
    ...
    # ODE residual: -u"(x) - om^2*u

    residual = -d2u - om*om*u
    interior_loss = torch.mean(residual ** 2)

    # calculating  loss for problem

    ivp_1 = u(torch.tensor([[0.]],requires_grad=True))  - 0.
    ivp_2 = du(torch.tensor([[1.0]],requires_grad=True)) - 1.
...
if __name__ == "__main__":
    ...
    y_true = torch.sin(a*x_test).numpy()
    plt.plot(x_test.numpy(), y_true, label= r'True Solution: $u^*(x) = sin(x)/om$', color='green')
