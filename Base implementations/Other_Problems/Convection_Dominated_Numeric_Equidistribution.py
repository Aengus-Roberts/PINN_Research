...
def compute_energy_loss(model, x, w, epsilon):
    ...
    alpha = 1/2 - 1/EPSILON
    b = 1/4 * (1-EPSILON/4)
    integrand = torch.exp(alpha*x) * ((epsilon/2)*du**2 + (b/2)*u**2 - torch.exp(-x/4)*u)
...
def get_quad_points(N=QUAD_NUMBER, type='uniform'):
    if type == 'uniform':
        ...
        N_R = N - N_L
        left = torch.linspace(0, 1-EPSILON, N_L)
...
def evaluate_equidistribution(model, method = 0):
    ...
    if method == 0:
        b = 1 / 4 * (1 - EPSILON / 4)
        d2u = (1/EPSILON - 1/2)*du + (b/EPSILON) * u - torch.exp(-X/4)/EPSILON #Cheating trick by rearranging ODE
    elif method == 1:
        du = torch.autograd.grad(u, X, grad_outputs=torch.ones_like(u),create_graph=True, retain_graph=True)[0]
...
def main():
    ...
    u2 = lambda x: np.exp(-x/4) * (x - (np.exp(-(1-x)/(EPSILON)) - np.exp(-1/EPSILON))/(1 - np.exp(-1/EPSILON)))
    ...
    plt.xlabel('x')
    plt.ylabel('u(x)')
    plt.legend(title='Quadrature Method:')
    title = r"Linear Spline, Numerically Equidistributed Knots, DRM Energy, ε = {:.2f}".format(EPSILON)
