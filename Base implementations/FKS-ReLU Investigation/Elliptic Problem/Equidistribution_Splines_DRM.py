...
class FKS(nn.Module):
    def __init__(self, knot_points):
        ...
        self.interior_knot_points = nn.Parameter(knot_points[1:-1])
        self.left_knot = knot_points[:1]
        self.right_knot = knot_points[-1:]

    @property
    def knot_points(self):
        return torch.cat([self.left_knot, self.interior_knot_points, self.right_knot])
...
def train_model(x, w):
    ...
    def trainParam(parameter):
        if parameter == 0:
            ...
            optimiser = optim.LBFGS([model.interior_knot_points], lr=0.01, max_iter=INNER_EPOCHS)
        ...
        if parameter == 0:
            ...
            optimiser.step(Equidistribution_closure)
        return model
...
def create_results(x_test, x, w, color='red', label=''):
    ...
    plt.scatter(model.knot_points.detach().numpy(), zeros)
