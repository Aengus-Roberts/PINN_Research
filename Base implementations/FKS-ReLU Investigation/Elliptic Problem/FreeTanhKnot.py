...
class FKS(nn.Module):
    def __init__(self, knot_points):
        ...
        self.tanh_coeff = nn.Parameter(torch.tensor([5.0], dtype=torch.float32))
        self.uniform_knots = knot_points

    def knot_points(self):
        return (torch.tanh(self.tanh_coeff * (self.uniform_knots - 0.5)) + 1) / 2
    ...
    def forward(self, x):
        ...
        # FKS: shape (N,K - 1)
        FKS = torch.zeros(len(x), len(self.uniform_knots) - 1, dtype=torch.float32, device=x.device)
...
def train_adam(x, w):
    ...
    def trainParam(parameter):
        if parameter == 0:
            ...
            optimiser = optim.Adam([model.tanh_coeff], lr=0.01)
...
def train_model(x, w):
    ...
    def trainParam(parameter):
        if parameter == 0:
            ...
            optimiser = optim.LBFGS([model.tanh_coeff], lr=0.01, max_iter=INNER_EPOCHS)
