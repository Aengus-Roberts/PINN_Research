...
def reference(x, epsilon):
    # Algebraically identical to the cosh formula, without overflow.
    left, right = np.exp(-x / epsilon), np.exp((x - 1) / epsilon)
    denominator = 1 + np.exp(-1 / epsilon)
    return 1 - (left + right) / denominator, (left - right) / (epsilon * denominator)
...
def main():
    parser = argparse.ArgumentParser(description='Compare saved elliptic solutions and report L2, H1-seminorm and boundary errors.')
    ...
    parser.add_argument('--output', type=Path, default=Path(__file__).resolve().parent/'elliptic_comparison')
    ...
    axes[0].set_title(r'$-\varepsilon^2 u^{\prime\prime}+u=1$, $u(0)=u(1)=0$, '+f'ε={args.epsilon:g}')
