...
def reference(x, epsilon):
    u = np.tanh(x / (epsilon * np.sqrt(2)))
    return u, (1 - u**2) / (epsilon * np.sqrt(2))
...
class SavedHats:
    def __init__(self, path, legacy_left_zero=False):
        ...
        if not np.all(np.diff(self.knots) > 0) or not np.allclose(self.knots[[0,-1]], [-1,1]):
            raise ValueError('Expected increasing knots covering [-1,1].')
...
class SavedCollocation:
    def __init__(self, path):
        ...
        if not np.allclose(self.knots[[0,-1]], [-1,1]):
            raise ValueError('Collocation data must cover [-1,1].')
...
def errors(model, breaks, epsilon, order):
    ...
    boundary, _ = model.evaluate(np.array([-1.,1.]))
    boundary = boundary - np.array([-1., 1.])
...
def main():
    parser = argparse.ArgumentParser(description='Compare saved Allen-Cahn solutions and report L2, H1-seminorm and boundary errors.')
    ...
    parser.add_argument('--output', type=Path, default=Path(__file__).resolve().parent/'allen_cahn_comparison')
    ...
    breaks = np.unique(np.r_[np.linspace(-1,1,max(101,int(np.ceil(20/args.epsilon))+1)),
    ...
    x = np.unique(np.r_[np.linspace(-1,1,10001), hats.knots])
    ...
    axes[0].set_title(r'$-\varepsilon^2 u^{\prime\prime}+u^3-u=0$, $u(-1)=-1,\ u(1)=1$, '+f'ε={args.epsilon:g}')
    axes[0].legend(fontsize=9)
    axes[1].set(xlabel='x',ylabel='Absolute error')
    for ax in axes:
        ax.grid(alpha=.25); ax.set_xlim(-1,1)
