...
def compute_energy_loss(model, x, w, epsilon, u_t):
    ...
    integrand = (0.5 * epsilon ** 2 * du ** 2+ 0.25 * LAMBDA * (1.0 - u ** 2) ** 2+ 0.5 * (u - u_previous) ** 2 / DT)
...
def evaluate_equidistribution(model, u_t, method=0):
    ...
    u_t = u_t(X)
    if method == 0:
        d2u = (LAMBDA * u * (1-u**2) - ((u - u_t)/DT)) / (EPSILON**2)
    elif method == 1:
...
def plot_spacetime_3d(
    ...
    ax.set_title("Allen–Cahn space-time solution")
...
def main():
    ...
    knots_0 = torch.linspace(-1, 1, KNOT_NUMBER, dtype=DTYPE)
    weights_0 = torch.rand(KNOT_NUMBER, dtype=DTYPE) * 2 - 1
    solvedModels = []
